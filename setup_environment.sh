...
sudo lxc exec demo -- apk add nginx
...
sudo lxc exec demo -- apk add net-tools postgresql
...
sudo lxc exec demo -- su postgres -c 'pg_ctl start -D /var/lib/postgresql/data -W;'
sleep 5
